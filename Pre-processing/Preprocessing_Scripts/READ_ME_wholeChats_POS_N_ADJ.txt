...
7) keeping words that are Parts-of-Speech (POS) of nouns and adjectives. 

The scripts and data/output files are:

processChatCSV_wholeChats_POS_N_ADJ.py - script to read chat data .csv file and writes 
...
OUTPUT Files: wholeChatsFilePOS_N_ADJ.csv and wholeChatsFilePOS_N_ADJ.txt
1) wholeChatsFilePOS_N_ADJ.csv with one chat per line formatted as:
   chat line # in original .csv, whole chat, and
2) wholeChatsFilePOS_N_ADJ.txt with only the processed text of each chat one per line
